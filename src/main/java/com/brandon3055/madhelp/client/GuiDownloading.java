...
public class GuiDownloading extends GuiScreen implements GuiYesNoCallback{
		...
		//Unzip and install the world
			...
			if (!content.isRegionDownload){
				...
				if (foundConflict){
					...
					if (!ContentHandler.worldFolder.renameTo(new File(ContentHandler.saveFolder, ContentHandler.worldFolder.getName()))) message = "Was unable to transfer world from temporary folder to saves folder...";
	...
	public void tryNewName(String newName) {
		...
		if (foundConflict){
			...
			if (!ContentHandler.worldFolder.renameTo(new File(ContentHandler.saveFolder, newName))) message = "Was unable to transfer world from temporary folder to saves folder...";
			try {FileUtils.deleteDirectory(ContentHandler.tempFolder); }
			catch (IOException e) { e.printStackTrace(); }
		}
...
//				renderFalling(tick);
			...
			if (fileSize == 0) progressText = I18n.format("gui.mad.starting.info");
				...
				progressText += " " + Math.round(progress*10000) / 100D + "% - " + DataUtils.formatFileSize(downloaded)+" / "+DataUtils.formatFileSize(fileSize);
			}
			fontRendererObj.drawStringWithShadow(progressText, width / 2 - fontRendererObj.getStringWidth(progressText) / 2, height/2 + 5, 0x00FFFF);
		}
		else if (installStage == 2){
			...
			if (message.equals("Download Failed")){
				fontRendererObj.drawSplitString("This downloader is not as sophisticated as most. If you are having trouble downloading the map try " +
						"downloading the map via the direct link. Then place the downloaded zip in the saves folder and run the installation process again. " +
						"(Or you could just unzip it your self)", 50, 200, width - 100, 0xFFFFFF);
			}
		}
