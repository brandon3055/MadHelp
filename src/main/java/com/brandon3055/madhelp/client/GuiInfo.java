...
public class GuiInfo extends GuiScreen {
    ...
    public void drawScreen(int x, int y, float f) {
        ...
        for (int i = 0; i < text.size(); i++) {
            @SuppressWarnings("unchecked") List<String> list = fontRenderer.listFormattedStringToWidth(text.get(i), width - 20);

            for (String text : list) {
                if (text.length() > 3 && text.substring(0, 3).equals("[c]")) {
                    text = text.substring(text.indexOf("[c]") + 3);
                    drawCenteredString(fontRenderer, text, width / 2, yPos, 0xFFFFFF);
                }
                else {
                    fontRenderer.drawString(text, 10, yPos, 0xFFFFFF);
                }
                yPos += 10;
            }
            yPos += 5;
