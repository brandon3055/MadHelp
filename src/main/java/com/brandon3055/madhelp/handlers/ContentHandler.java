...
public class ContentHandler {
	...
	public static void readCachedVersions(){
		...
		for (DownloadableContent content : compiledContentList) {
			if (!cachedVersions.containsKey(content.displayName) || cachedVersions.get(content.displayName) != content.releaseNumber) updates.put(content.displayName, !cachedVersions.containsKey(content.displayName));
		}
	}
	...
	public static void updateCachedVersions(){
		...
		for (DownloadableContent content : compiledContentList) versions.put(content.displayName, content.releaseNumber);
	...
	private static void addDefaultFiles(){
		...
		if (!splashScreenFile.exists() && ConfigHandler.enableSplashScreen){
			try
			{
				...
				writer.name("infoText").beginArray()
						.value("[c]This is an example splash screen")
						.value("If you are seeing this then ether MadHelp has just been loaded for the first time or the splash screen has not been properly configured by the pack creator.")
						.value("This screen uses the same formatting options as the map info screens.");
				writer.endArray();
				writer.endObject();
				writer.close();
			}
	...
	public static class DownloadableContent {
		...
		public boolean isValid(){
			if (StringUtils.isNullOrEmpty(displayName) || StringUtils.isNullOrEmpty(fileName) || description == null || StringUtils.isNullOrEmpty(downloadUrl) || (isRegionDownload && StringUtils.isNullOrEmpty(regionInputPath) && StringUtils.isNullOrEmpty(regionOutputPath))) return false;
			return true;
		}
	...
	public static class DownloadThread extends Thread {
		...
		@Override
		public void run() {
			downloadedFile = new File(FileHandler.mcDirectory, "saves/"+content.fileName);
