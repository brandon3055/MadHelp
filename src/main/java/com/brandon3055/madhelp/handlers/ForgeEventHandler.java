...
public class ForgeEventHandler {
	...
	public void guiOpen(GuiScreenEvent.InitGuiEvent event){
		if (event.getGui() instanceof GuiMainMenu){
			...
			if (ContentHandler.updates.size() > 0) bText += TextFormatting.RED + " " + I18n.format("gui.mad.new.button");
	...
	private static class GuiLane extends GuiShareToLan {
		...
		public void drawScreen(int p_73863_1_, int p_73863_2_, float p_73863_3_)
		{
			...
			for (String s : ConfigHandler.disableCheatMessage){
				this.drawCenteredString(this.fontRendererObj, s, this.width / 2, yPos, 16777215);
				yPos+=10;
			}
